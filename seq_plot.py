...
with torch.no_grad():
    ...
    for epoch in range(args.num_plot):
        ...
        net.init_hidden()
        hidden_seq, output, context_seq = net(input) # ADDED context_seq

        hidden = hidden_seq.squeeze()
        context = context_seq.squeeze() # NEW LINE

        all_hid.append(hidden)
        all_cont.append(context) # NEW LINE
    ...
    all_context = torch.cat(all_cont, dim=0)
    for k in range(args.hid):
        for j in range(k):
            plt.scatter(all_context[:,j], all_context[:,k], c=all_state,
                cmap='viridis', vmin=0, vmax=max_state)
            plt.savefig('./plot/%s_%s%d_%d%d_context.jpg' %(args.lang,args.model,args.hid,j,k))
            plt.show()
