...
class lang_anb2n:
    def __init__(self,num_class=2,length=4):
    ...
    def print_outputs(self,epoch,seq,state,hidden,target,output, context):
        hidden_np = hidden.squeeze().numpy()
        context_np = context.squeeze().numpy() #NEW
        ...
        print('hidden activations, output probabilities and context:') # MODIFIED TO ADD CONTEXT
        for k in range(len(state)-1):
            print(self.chars[seq[k]],hidden_np[k,:],prob_out_np[k,:],context_np[k,:]) # MODIFIED TO ADD CONTEXT
        print('epoch: %d' %epoch)
        print(' loss: %1.4f' %torch.mean((prob_out - target)
                                        *(prob_out - target)))
